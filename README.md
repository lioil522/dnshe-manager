...
- **主机记录自动转相对名**：上游 `dns_records/list` 读出来的 `name` 是**完整域名**（`ipv6.1.cd`），而写接口只接受 `@` 或相对名，原样回填提交会被拒（`record name must be @ or a relative record name`）。因此编辑框回填的是相对名（`@` / `jp`），后端 `normalizeDnsRecordName` 还会对创建 / 修改 / 批量三条写路径统一兜底：完整域名剥成相对名、带尾点去点、中文子域转 `xn--`。
