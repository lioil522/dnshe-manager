...
export interface SubdomainInfo {
   ...
   * 无需额外调用。
   *
   * NOTE: 这是判断「域名是否支持按线路（运营商/地域）解析」目前唯一可查的信号。
   * API 文档没有任何线路能力字段（`fields` 参数枚举了子域名的全部可选字段，里面
   * 没有），也没有根域名列表或 capability 接口；官网自己也是把「目前只有 us.ci 与
   * cn.mt 支持」写死在文案里的。实测同一账号下 us.ci / cn.mt 的该值为 1，其余
   * 7 个根域名为 7 或 8，两组取值无交集。
   *
   * 但文档从未说明这个字段的语义，属实测相关性而非契约，所以支持名单做成可配置
   * （见前端 DNSHE_LINE_PROVIDERS）：厂商在同一服务商下新增根域名能自动识别，
   * 若另开一个也支持线路的服务商则需要手工补一条。
   */
