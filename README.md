...
这些词表照搬西部数码域名查询客户端反编译源码的 `Words.DomainWords()`，口径是「照搬源码 + 修正明确缺陷」，每处增删都在源码行内标注（详见 [规则词表数据来源与校验](#-91-规则词表数据来源与校验)）。邮编 3173 条同样照搬源码生成而非手工转录。因此 `{声母}{韵母}` 产出的是 480 个**可发音音节**而不是字母对，`{豹子}` / `{顺子}` 不含数字 `4` 也是原表的既有取舍。
