...
- **两步验证 (2FA / TOTP)**：支持绑定标准身份验证器（Google Authenticator / Microsoft Authenticator / Bitwarden），扫描 Base32 二维码完成 6 位动态口令验证。开启 2FA 后，登录页会**直接展示动态码输入框**（通过鉴权状态接口预判），无需先提交用户名密码。
...
│   ├── src/
│   │   ├── App.tsx             # 前端主界面组件 (包含面板卡片、域名管理、DNS模态框、安全设置、未读告警铃铛)
│   │   ├── main.tsx            # 应用挂载入口
...
│   ├── index.ts                # RESTful API 路由定义 (基于 Hono.js 框架)、鉴权中间件、API 缓存读写与 Worker 入口
│   ├── dnshe.ts                # DNSHE 官方 REST API V2.0 客户端 SDK (HMAC/请求签名逻辑)
│   ├── db.ts                   # D1 数据库操作类、AES-GCM 加解密算法、API 缓存方法与安全日志封装
...
- 批量域名查重（规则生成前缀逐个 WHOIS）是唯一的高频调用场景，请合理控制生成数量，避免触发滥用判定。
